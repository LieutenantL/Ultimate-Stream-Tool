...
async function getData(scInfo) {
	...
	let caster1 = caster[0].name;
	twitter1 = caster[0].twitter;
	twitch1 = caster[0].twitch;
	let caster2 = caster[1].name;
...
function updateChar(pCharacter, pSkin, pNum, charInfo, mainMenu, startup = false) {
	...
	charEL.src = charPath + 'HQ Stock Icons/' + pCharacter + '/' + pSkin + '.png';

	//             x, y, scale
	let charPos = [10, -20, .85];
	...
	charEL.decode().then(
		...
		charEL.src = charPathBase + 'HQ Stock Icons/Random/Random.png';
		fadeInChara(charImg[pNum], charPos[2], startup);
	})
